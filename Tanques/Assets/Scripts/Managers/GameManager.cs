...
using UnityEngine.UI;public class GameManager : MonoBehaviour
{    public int m_NumRoundsToWin = 5; //Número de rondas que un jugador debe ganar para ganar el juego
    ...
    private TankManager GetGameWinner()
    {
        ...
        for (int i = 0; i < m_Tanks.Length; i++)
        {
            ...
            if (m_Tanks[i].m_Wins == 3)
            {
                Ganador_ = m_RoundWinner.m_ColoredPlayerText + " GANA LA PARTIDA!";
                return m_Tanks[i];
            }
        }
        // Si no, devuelvo null.
    ...
    private string EndMessage()
    {
        ...
        if (m_GameWinner != null)
        {
            message = m_GameWinner.m_ColoredPlayerText + " GANA EL JUEGO " +"-"+ " PUNTAJE: " + contt.ToString();
